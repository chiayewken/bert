...
def create_model(is_training, input_ids, input_mask, segment_ids, labels,
  ...
  with tf.variable_scope("loss"):
    ...
    return run_regressor.get_model_outputs(output_layer, labels)
...
def main(_):
  ...
  processors = {
      "cola": run_regressor.ColaProcessor,
      "mnli": run_regressor.MnliProcessor,
      "mrpc": run_regressor.MrpcProcessor,
  }
  ...
  if FLAGS.do_train:
    train_features = run_regressor.convert_examples_to_features(
        train_examples, label_list, FLAGS.max_seq_length, tokenizer)
    tf.logging.info("***** Running training *****")
    tf.logging.info("  Num examples = %d", len(train_examples))
    tf.logging.info("  Batch size = %d", FLAGS.train_batch_size)
    tf.logging.info("  Num steps = %d", num_train_steps)
    train_input_fn = run_regressor.input_fn_builder(
  ...
  if FLAGS.do_eval:
    ...
    eval_features = run_regressor.convert_examples_to_features(
    ...
    eval_input_fn = run_regressor.input_fn_builder(
  ...
  if FLAGS.do_predict:
    ...
    run_regressor.file_based_convert_examples_to_features(
    ...
    predict_input_fn = run_regressor.file_based_input_fn_builder(
